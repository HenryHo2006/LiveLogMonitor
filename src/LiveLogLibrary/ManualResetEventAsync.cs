...
namespace LiveLogMonitor
{
    ...
    public sealed class ManualResetEventAsync
    {
        ...
        public void Reset()
        {
            ...
            Interlocked.CompareExchange(ref this._completionSource, new TaskCompletionSource<bool>(), currentCompletionSource);
        }
        ...
        private async Task<bool> AwaitCompletion(int timeoutMs, CancellationToken token)
        {
            ...
            using (timeoutToken)
            {
                ...
                Task delayTask = Task.Delay(timeoutMs, timeoutToken.Token).ContinueWith((result) => { var e = result.Exception; },
